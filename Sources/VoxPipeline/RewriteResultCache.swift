...
public actor RewriteResultCache {
    public static let shared = RewriteResultCache(maxEntries: 128, ttlSeconds: 600, maxCharacterCount: 1_024)
    ...
    public init(maxEntries: Int, ttlSeconds: TimeInterval, maxCharacterCount: Int) {
        self.maxEntries = max(1, maxEntries)
        self.ttlSeconds = max(0.001, ttlSeconds)
        self.maxCharacterCount = max(1, maxCharacterCount)
    }

    public func value(for transcript: String, level: ProcessingLevel, model: String) -> String? {
    ...
    public func store(_ value: String, for transcript: String, level: ProcessingLevel, model: String) {
